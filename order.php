	...
	<div class="main">
		...
		<section class="orders">
				...
				<div class="box-container">
					<?php 
						// Query to select user orders from the database
						$select_orders = $conn->prepare("SELECT * FROM `orders` WHERE user_id = ? ORDER BY date DESC");
						$select_orders->execute([$user_id]);
						// Check if there are orders for the user
						if ($select_orders->rowCount()>0) {
							while($fetch_order = $select_orders->fetch(PDO::FETCH_ASSOC)){
								// Query to select product details for each order
								$select_products = $conn->prepare("SELECT * FROM `products` WHERE id=?");
								$select_products->execute([$fetch_order['product_id']]);
								// Check if there are products for the order
					...
					<?php 
							 ...
							 // Display message if no orders are placed yet
